...
- 403 com código de negócio levanta `AraraForbiddenError` (ou `AraraPlanFeatureLockedError` para `PLAN_FEATURE_LOCKED`, com `feature`, `current_plan`, `upgrade_to`); 403 sem código e 401 levantam `AraraAuthError`; 422 levanta `AraraUnprocessableError`.
- POST/PATCH sem `Idempotency-Key` não são mais repetidos em timeout, erro de conexão, 5xx ou 429.

### Novo
- `auth.me()` (`GET /auth/me`, exige chave ADMIN).
...
- `messages.send_batch` (até 1000 destinatários) e `messages.list_by_batch`.
